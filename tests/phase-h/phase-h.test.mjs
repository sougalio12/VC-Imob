...
test('H19 Supabase auth persists across PWA restarts without native plaintext storage', () => { const auth = readFileSync('crm/js/supabase.js', 'utf8'); assert.match(auth, /localStorage/); assert.match(auth, /refresh_token/); assert.doesNotMatch(auth, /Preferences/); });
