...
test('stage request uses secured undo RPC with timestamp CAS and allowed field',async()=>{let call;const r=runtime({isDemoMode:()=>false,supabaseRequest:async(path,options)=>{call={path,options};return {lead:{...fixture()[0],stage:'visita'},undo_id:'undo-a'};}});vm.runInContext("getActiveOrganizationId=async()=> 'org-a'",r);await r.moveKanbanLead(fixture()[0],'visita');assert.equal(call.path,'/rest/v1/rpc/change_lead_with_undo');assert.deepEqual(JSON.parse(call.options.body),{target_organization:'org-a',target_lead:'a',target_expected_updated_at:fixture()[0].updated_at,target_field:'stage',target_value:'visita'});});
test('RLS or empty undo RPC response raises conflict',async()=>{const r=runtime({isDemoMode:()=>false,supabaseRequest:async()=>null});vm.runInContext("getActiveOrganizationId=async()=> 'org-a'",r);await assert.rejects(r.moveKanbanLead(fixture()[0],'visita'));});
test('quick editor rejects zero-row writes and carries exact revision',async()=>{let call;const r=runtime({isDemoMode:()=>false,supabaseRequest:async(path,options)=>{call={path,options};return [];}});vm.runInContext("getActiveOrganizationId=async()=> 'org-a'",r);await assert.rejects(r.updateLead('a',{stage:'visita'},fixture()[0].updated_at));assert.equal(call.path,'/rest/v1/rpc/save_crm_lead');assert.equal(JSON.parse(call.options.body).target_expected_updated_at,fixture()[0].updated_at);});
