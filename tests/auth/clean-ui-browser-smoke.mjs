...
test("CLEAN-BROWSER real mobile widths contain auth forms and Team cards",{timeout:120000,skip:!existsSync(chrome)||process.env.VC_IMOB_BROWSER_SMOKE!=="1"},async()=>{
  ...
  try{
    ...
    for(const width of viewports){
      ...
      const editorState=await evaluate(`(()=>{const progress=document.querySelector(".document-progress"),rect=progress.getBoundingClientRect();return{steps:document.querySelectorAll(".document-progress button").length,visiblePanels:[...document.querySelectorAll(".document-step-panel")].filter(item=>!item.hidden).length,touch:[...document.querySelectorAll(".document-progress button,.document-editor-actions button")].filter(item=>!item.hidden).every(item=>Math.max(item.getBoundingClientRect().height,parseFloat(getComputedStyle(item).minHeight)||0)>=43.5),horizontal:document.documentElement.scrollWidth<=document.documentElement.clientWidth+1,progressContained:rect.left>=-1&&rect.right<=document.documentElement.clientWidth+1,progressScrollable:progress.scrollWidth>progress.clientWidth}})()`);
      assert.equal(editorState.steps,6);assert.equal(editorState.visiblePanels,1);assert.equal(editorState.touch,true,JSON.stringify(editorState));assert.equal(editorState.horizontal,true,JSON.stringify(editorState));assert.equal(editorState.progressContained,true,JSON.stringify(editorState));assert.equal(editorState.progressScrollable,true,JSON.stringify(editorState));await evaluate(`closeModal()`);
    }
  }finally{
