...
test("CLEAN06 the clean system preserves mobile ergonomics and reduced motion",()=>{
  assert.match(signup,/clean-system\.css\?v=polish-hotfix-20260922/);assert.match(clean,/min-height: 44px/);assert.match(clean,/prefers-reduced-motion/);
...
test("CLEAN08 screens have subtle contextual accents and Mais reuses the official PWA icon",()=>{
  ...
  assert.match(serviceWorker,/vc-imob-shell-polish-hotfix-20260922/);
});
