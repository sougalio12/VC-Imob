...
async function renderCurrentView() { const view = document.body.dataset.view || "dashboard"; const root = document.getElementById("crmContent"); root.replaceChildren(createElement("p", { className: "muted", text: "Carregando…" })); try { if (view === "dashboard") await renderDashboard(root); if (view === "leads") await renderLeads(root); if (view === "kanban") await renderKanban(root); if (view === "properties") await renderProperties(root); if (view === "agenda") await renderAgenda(root); } catch (error) { root.replaceChildren(createEmptyState("Não foi possível carregar esta área", error.message || "Verifique sua conexão e tente novamente.")); } }
