...
async function prepareAssistantAction(kind,payload){if(isDemoMode())return{id:`demo-${crypto.randomUUID()}`,action_kind:kind,payload,status:"pending",expires_at:new Date(Date.now()+900000).toISOString()};return callCrmRpc("prepare_assistant_action",{target_organization:await getActiveOrganizationId(),target_kind:kind,target_payload:payload});}
async function confirmAssistantAction(id){if(isDemoMode())return{kind:"demo",entity_id:id};return callCrmRpc("confirm_assistant_action",{target_preview:id});}
async function getLeadCommercialMemory(leadId){if(isDemoMode())return[];const org=await getActiveOrganizationId();return supabaseRequest(`/rest/v1/lead_commercial_memory?organization_id=eq.${encodeURIComponent(org)}&lead_id=eq.${encodeURIComponent(leadId)}&archived_at=is.null&select=*&order=created_at.desc`);}
async function addLeadCommercialMemory(leadId,kind,content,structuredData={}){if(isDemoMode())return{id:`memory-${crypto.randomUUID()}`,lead_id:leadId,memory_kind:kind,content,structured_data:structuredData};const org=await getActiveOrganizationId(),session=await getValidSession();return(await supabaseRequest("/rest/v1/lead_commercial_memory",{method:"POST",headers:{Prefer:"return=representation"},body:JSON.stringify([{organization_id:org,lead_id:leadId,memory_kind:kind,content,structured_data:structuredData,created_by:session.user.id}])}))[0];}
async function getLeadCommercialSummary(leadId){if(isDemoMode())return null;return callCrmRpc("get_lead_commercial_summary",{target_lead:leadId});}
