async function openKanbanDetail(lead, context) {
  ...
  card.append(await renderLeadInsights(lead, context));
  ...
  async function loadHistory() {
    ...
    else activity.value.forEach(event => { const meta = event.metadata || {}; const messages = { lead_note_created: "Nota adicionada", lead_interest_added: `Interesse adicionado: ${meta.property_code || "imóvel"}`, lead_interest_removed: `Interesse removido: ${meta.property_code || "imóvel"}`, activity_created: "Acompanhamento criado", activity_rescheduled: "Acompanhamento reagendado", activity_status_changed: `Acompanhamento: ${activityStatusLabel(meta.new_status)}` }; const message = event.action === "lead_stage_changed" ? `Etapa: ${stageLabel(meta.previous_stage)} → ${stageLabel(meta.new_stage)}` : messages[event.action] || `Responsável: ${context.memberName(meta.previous_assigned_to)} → ${context.memberName(meta.new_assigned_to)}`; history.append(createElement("p", { text: `${formatDate(event.created_at, true)} — ${message}` })); });
  }
